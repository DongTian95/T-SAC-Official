...
class AbstractAgent(ABC):
    def __init__(self,
                 ...
                 **kwargs):
        # Agent components
        # comment out because can cause debug difficulty
        # self.policy = policy
        # self.critic = critic
        ...
        self.total_iterations = kwargs.get("total_iterations",
                                           50000)  # Only used for schedule lr
    ...
    def save_agent(self, log_dir: str, epoch: int):
        ...
        if isinstance(self.critic_optimizer, list):
            ...
            with open(critic_optimizer_path, "wb") as critic_opt_file:
                for idx in range(len(self.critic_optimizer)):
                    torch.save(self.critic_optimizer[idx].state_dict(), critic_opt_file)


        elif not isinstance(self.critic_optimizer, tuple):
            critic_optimizer_path = \
                util.get_training_state_save_path(log_dir, "critic_optimizer",
                                                  epoch)
            with open(critic_optimizer_path, "wb") as critic_opt_file:
