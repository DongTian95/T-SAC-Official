...
class TrajectoryPolicyGPT(nn.Module):
    def __init__(self, **config):
        super().__init__()

        config = Dict(config)
        self.config = config
        self.no_layer_norm = config.no_layer_norm

        module_dict = dict(
            state_encoder=nn.Linear(config.state_dim, config.n_embd, bias=False),
            info_encoder=nn.Linear(config.info_dim, config.n_embd, bias=False),
            pos_enc=nn.Embedding(config.block_size, config.n_embd),  # pos encodings, check init correctly todo
        ...
        self.output_layer = nn.Linear(config.n_embd, config.dim_out, bias=False)
    ...
    def forward(self, c_state, infos, idx_c, idx_i, d_state=None, idx_d=None):
        """
        ...
        Args:
            ...
            idx_i: time step indices of actions, [*add_dim, num_actions]
        ...
        """
        # this part is to improve capability
        have_decision_state = True if d_state is not None else False

        # fixme, check if action is None?

        t = infos.size(-2)

        assert t + 2 <= self.config.block_size

        # data embedding, # fixme check if time dim is given
        if have_decision_state:
        ...
        info_emd = self.transformer.info_encoder(infos)
        # Shape [*add_dim, 2 + t, n_embed]
        if have_decision_state:
            seq_emb = torch.cat([d_state_emd, state_emd, info_emd], dim=-2)
        else:
            seq_emb = torch.cat([state_emd, info_emd], dim=-2)

        # Shape [*add_dim, 2 + t]
        if have_decision_state:
            seq_pos = torch.cat([idx_d[..., None], idx_c[..., None], idx_i], dim=-1)
        else:
            seq_pos = torch.cat([idx_c, idx_i], dim=-1)  # fixme: size has problem

        # Shape [*add_dim, t + 1, n_embed]
        seq_pos = seq_pos.long()    # The embedding function expects the indices to be integers
        ...
        # fixme, should I do x[..., 1:] before the layer norm?
