...
class AbstractGaussianPolicy(ABC):
    def __init__(self,
        ...
        # Net normalization
        self.mean_layer_norm = mean_net_args.get("layer_norm", False)
        if "layer_norm" in mean_net_args:
            del mean_net_args["layer_norm"]
        self.variance_layer_norm = variance_net_args.get("layer_norm", False)
        if "layer_norm" in variance_net_args:
            del variance_net_args["layer_norm"]
        ...
        # feed mean to variance net
        self.feed_mean_to_variance = bool(kwargs.get("feed_mean_to_variance", False))
        ...
        self.log_min_std = float(kwargs.get("log_min_std", -20.0))
    ...
    def _create_network(self):
        ...
        if self.contextual_cov:
            if not self.feed_mean_to_variance:
                ...
                self.variance_net = MLP(name=self._policy_net_type + "_variance",
                                        dim_in=self.dim_in + self.dim_out,
                                        dim_out=dim_out_var,
                                        hidden_layers=util.mlp_arch_3_params(
                                            **self.variance_net_args),
                                        init_method=self.init_method,
    ...
    def _vector_to_cholesky(self, cov_val: torch.Tensor):
        ...
        if not self.std_only:
            ...
            diag_cov_val = torch.clamp(diag_cov_val, min=self.log_min_std, max=2)
            diag_cov_val = util.gsde_std(diag_cov_val)  # treat as log_std
            params_L = util.build_lower_matrix(diag_cov_val, off_diag_cov_val)  # off_diag_cov_val is None
