...
class TrajectoryQfunctionGPT(nn.Module):
    ...
    def forward(self, c_state, actions, idx_c, idx_a, d_state=None, idx_d=None, no_absolute_idx=False):
        """
        ...
        elif no_absolute_idx:
            seq_pos = torch.arange(0, t + 1, dtype=torch.long, device=self.device).unsqueeze(0)
        else:
            seq_pos = torch.cat([idx_c, idx_a], dim=-1)
