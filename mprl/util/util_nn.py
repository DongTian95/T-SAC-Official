...
def get_act_module(key: Optional[str]) -> Optional[nn.Module]:
    """
    Returns an nn.Module activation for the given key,
    ...
    return act_map.get(key)  # returns the module or None
...
def initialize_weights(model: nn.Module, initialization_type: str,
    ...
    for p in model.parameters():
        if initialization_type == "normal":
            ...
            if len(p.data.shape) >= 2:
                nn.init.kaiming_normal_(p.data, mode=initialization_type, a=0.01,
...
class MLP(nn.Module):
    def __init__(self,
        ...
        self.act_func_hidden = get_act_module(act_func_hidden)
        self.act_func_last_type = act_func_last
        self.act_func_last = get_act_module(act_func_last)
        self.init_method = init_method
        self.out_layer_gain = out_layer_gain
        self.add_layer_norm = kwargs.get("layer_norm", False)
        self.out_layer_norm = kwargs.get("out_layer_norm", False)
    ...
    def _create_network(self):
        ...
        initialize_weights(input_layer, self.init_method, activation=self.act_func_hidden_type)
        mlp.append(input_layer)
        if self.add_layer_norm:
            mlp.append(nn.LayerNorm(input_layer.out_features, device=self.device, dtype=self.dtype))
        if self.act_func_hidden is not None:
            mlp.append(self.act_func_hidden)

        # Add other hidden layers
        for i in range(1, len(self.hidden_layers)):
            hidden = nn.Linear(in_features=input_layer.out_features,
                               out_features=self.hidden_layers[i],
                               dtype=self.dtype, device=self.device)
            initialize_weights(hidden, self.init_method, activation=self.act_func_hidden_type)
            mlp.append(hidden)
            if self.add_layer_norm:
                mlp.append(nn.LayerNorm(hidden.out_features, device=self.device, dtype=self.dtype))
            if self.act_func_hidden is not None:
                mlp.append(self.act_func_hidden)

        # Add output layer
        output_layer = nn.Linear(in_features=hidden.out_features,   # hidden will be overwritten during loop
        ...
        initialize_weights(
            ...
            **({"activation": self.act_func_last_type} if self.act_func_last is not None else {})
        )
        mlp.append(output_layer)
        if self.out_layer_norm:     # to make it compatible to base_net
            mlp.append(nn.LayerNorm(self.dim_out, device=self.device, dtype=self.dtype))
        if self.act_func_last is not None:
            mlp.append(self.act_func_last)
