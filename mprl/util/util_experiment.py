...
class RLExperiment:
    ...
    def _process_train_rep_config_file(self, config_obj):
        ...
        for i, rep_config in enumerate(exp_configs):
            ...
            if rep_config.get("seed")=="auto" or rep_config.get("seed")=="Auto":
                init_seed = 0
            else:
                init_seed = int(rep_config.get("seed"))

            # Set random seed to the repetition number
            util.set_value_in_nest_dict(rep_config, "seed",
                                        rep_config['_rep_idx'] + init_seed)
            print("the current seed is: ", rep_config['seed'])
